...
TEST(PIDcontrollerTest, TestCompute) {
  ...
  // Since the target setpoint is 50.0 and actual velocity is 100.0, the velocity should decrease
  EXPECT_LT(result, 100.0);  // Expect the new velocity to be less than 100.0, moving toward the setpoint
}
...
TEST(PIDcontrollerTest, TestSetActualVelocity) {
  ...
  EXPECT_EQ(pid.getActualVelocity(), 100.0);  // This should pass because actual velocity is 100.0
}
